...
export class SkinnedMeshRenderer extends MeshRenderer {
  ...
  private static _jointTextureProperty = Shader.getPropertyByName("u_jointTexture");
  ...
  @ignoreClone
  public jointTexture: SampledTexture2D;
   ...
   * @internal
   */
  _updateShaderData(viewMatrix: Matrix, projectionMatrix: Matrix) {
    super._updateShaderData(viewMatrix, projectionMatrix);
  ...
  createJointTexture() {
    if (!this.jointTexture) {
      ...
      this.jointTexture = new SampledTexture2D(engine, 4, this.jointNodes.length, 1, "rgba32float");
      this.jointTexture.magFilterMode = "nearest";
      this.shaderData.enableMacro("O3_USE_JOINT_TEXTURE");
      this.shaderData.setSampledTexture(
        SkinnedMeshRenderer._jointTextureProperty,
