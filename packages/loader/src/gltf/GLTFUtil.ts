import { TypedArray } from "@arche-engine/core";
...
import { VertexAttribute } from "@arche-engine/core/src";
...
export class GLTFUtil {
  ...
  static createVertexElement(semantic: string, accessor: IAccessor, index: number): VertexAttribute {
    const size = GLTFUtil.getAccessorTypeSize(accessor.type);
    return new VertexAttribute(0, GLTFUtil.getElementFormat(accessor.componentType, size, accessor.normalized), index);
  }

  static getIndexFormat(type: AccessorComponentType): GPUIndexFormat {
    switch (type) {
      case AccessorComponentType.UNSIGNED_SHORT:
        return "uint16";
      case AccessorComponentType.UNSIGNED_INT:
  ...
  static getElementFormat(type: AccessorComponentType, size: number, normalized: boolean = false): GPUVertexFormat {
