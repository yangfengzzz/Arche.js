...
export class Gltf2 {
  ...
  getSkin(id: string | number | undefined): Skin | null {
    ...
    let i: number, bi: number, ni: number, joint: SkinJoint, node: Record<string, any>;
    ...
    let j: number;
    for (i = 0; i < js.joints.length; i++) {
      // Joint Points t a Node
      ni = js.joints[i];
      // Get that Joint's Node
      node = json.nodes[ni];
      ...
      if (node?.children?.length) {
        for (j = 0; j < node.children.length; j++) {
          // Joint Node Children Index, get their mapped joint index.
          bi = jMap.get(node.children[j]);
          // With Child Joint Index, Save this Index as its parent.
          if (bi != undefined) skin.joints[bi].parentIndex = i;
  ...
  getAnimation(id: string | number | undefined): any {
    ...
    // Channel
    let ch: any;
    // Joint Index it Effects
    let jointIdx: number;
    // Sampler Object ( Input:Timestamp Array, Output:Value Array )
    let sampler: any;
    // Accessor tmp var
